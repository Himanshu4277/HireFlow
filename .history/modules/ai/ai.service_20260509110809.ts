...
1. Full Name
2. Skills (IMPORTANT: extract ALL skills, even soft skills like teaching, communication, leadership)
...
DO NOT leave skills empty. Infer from context if needed.
...
`
    ...
    if (!response.ok) {
      ...
      throw new Error(aiData.error?.message || "AI failed");
    }
    ...
    const clean = content.replace(/```json|```/, "").trim();
