          ...
          {role !== "recruiter" ? (
            <div className="flex flex-col gap-1">
              <div className="flex gap-2">
                <button
                  onClick={() => router.back()}
                  className="px-5 py-2.5 rounded-lg text-sm bg-white/5 hover:bg-white/10 text-slate-300 border border-white/[0.08] transition-colors"
                >
                  ← Go Back
                </button>
                <button
              ...
              {status === "pending" && (
                <p className="text-xs text-slate-500">We'll notify you once reviewed</p>
              )}
              {status === "accepted" && (
                <p className="text-xs text-emerald-500">🎉 Congrats! Check your email for next steps</p>
              )}
              {status === "rejected" && (
                <p className="text-xs text-slate-500">Keep applying — the right role is out there</p>
              )}
            </div>
          ) : (
