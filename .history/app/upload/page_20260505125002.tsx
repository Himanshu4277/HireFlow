...
export default function ResumeUploader() {
    ...
    const handleClear = () => {
     ...
     ─── Save analyzed resume to localStorage ─────────────────────────────────
    const saveResumeToHistory = (file: File, normalized: any) => {
        try {
            const score = normalized.score ?? Math.floor(Math.random() * 30) + 65;  fallback random 65–94
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            setResultStore(normalized);
             ...
             ─── 3. Save to localStorage for /resumes page ───────────────────
            saveResumeToHistory(uploadedFile, normalized);

             ─── 4. Smart skill fallback ─────────
