    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const jobRes = await fetch("/api/jobs/by-skills", {
                ...
                body: JSON.stringify({ skills: normalized.skills }),
            });

            const jobText = await jobRes.text();
            console.log("RAW JOB RESPONSE:", jobText);

            let jobData;
            try {
                jobData = JSON.parse(jobText);
            } catch {
                console.error("Jobs API returned invalid JSON");
                setLoading(false);
                return;
            }
            ...
            if (jobData.success) {
                ...
                localStorage.setItem("jobs", JSON.stringify(jobData.jobs));
            }
